...
class VitaMultiScaleMaskedTransformerDecoder(nn.Module):
    ...
    def forward_base_train(self, x, src, pos, size_list, mask_features):
        ...
        outputs_class, outputs_mask, attn_mask, frame_query = self.forward_prediction_heads(output, self.class_embed, self.mask_embed, mask_features,
    ...
    def forward_new_train(self, x, src, pos, size_list, mask_features):
        ...
        # prediction heads on learnable query features
        outputs_class_p, outputs_mask_p, attn_mask_p, frame_query_p = self.forward_prediction_heads(
            output_p,
            self.class_embed,
            self.mask_embed if not self.prompt_mask_mlp else self.prompt_mask_embed[-1],
            #### self.prompt_mask_embed[-1]
            mask_features,
            attn_mask_target_size=size_list[0],
            qdims=query_dims,
        )

        frame_queries.append(frame_query_p)
    ...
    def forward_infer(self, x, src, pos, size_list, mask_features):
        ...
        for i in range(self.num_layers):
            ...
            #####################################  grad_cross_attention  #####
            #if i == 0:    ################  0
                #self.g_prefix_feature[0] = {"key": output}   #####   iiiiiiiiii
            ####################################
    ...
    def forward(self, x, mask_features, clip_mask_features, mask = None):
        ...
        if not self.training:
            out, frame_queries = self.forward_infer(x, src, pos, size_list, mask_features)

        else:
            if self.num_prompts > 0 and not self.old_model:
                out, frame_queries = self.forward_new_train(x, src, pos, size_list, mask_features)

            else:
                out, frame_queries = self.forward_base_train(x, src, pos, size_list, mask_features)


        num_layer         = self.vita_last_layer_num if self.training else 1   #3
    ...
    def forward_prediction_heads(self, output, class_embed, mask_embed, mask_features, attn_mask_target_size, qdims=None):
        ...
        outputs_mask = torch.einsum("bqc,bchw->bqhw", m_embed, mask_features)
